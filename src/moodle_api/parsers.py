    ...
    for tag in soup.find_all("input"):
        ...
        for i in to_iterate[0].contents:
            if _contains_answer(i):
                ...
                correct_answers.update({tag["name"].split(":")[1]: correct_ans})
...
def parse_radio_button_answers(soup):
    correct_answers = []
    radio_tags = soup.find_all(
        "input", {"type": "radio", "id": re.compile(r"q\d+:\d.*")}
    )
    if not radio_tags:
        return {}
    ...
    for tag in radio_tags:
        if (
            tag.next.text == correct_answers[i][1]
            and remove_prefix(remove_last_digit(tag["id"])) == correct_answers[i][0]
        ):
        ...
        for i in tag[0].parent.find_all():
            for content in i.contents:
                if _contains_answer(content):
                    ...
                    for option in soup.find_all("select", {"name": field})[0].find_all(
                        "option"
                    ):
                        if option.text == correct_ans:
                            correct_ans = option["value"]
                    correct_answers.update({field.split(":")[1]: correct_ans})
    return correct_answers
...
def parse_checkbox_answers(soup):
    ...
    for i in tmp:
        right_answers.append(
            [x.strip() for x in i.contents[0].split(":")[1].split(",")]
        )
    for field in task_fields:
...
def parse_answers(soup):
    result = dict()
    handlers = [
        parse_plain_text_answers,
        parse_radio_button_answers,
        parse_picker_answers,
        parse_checkbox_answers,
        parse_one_string_answers,
    ]
    while handlers:
