...
class SummaryPage(Page):
    def best_attempt_id(self):
        ...
        if best_attempt:
            try:
                best_url = best_attempt.find(
                    "a", {"title": "Просмотр своих ответов этой попытки"}
                )["href"]
            except TypeError:
                # Если есть незавершенная попытка, он не сможет найти 'href' у следующего тега
                # Поэтому мы просто пометим best_attempt в None
                logger.info(
                    "first own attempt and not finished. `best_url` is set to None"
                )
...
class RunningAttemptPage(Page):
    ...
    def id(self) -> str:
        if not self._id:
            try:
                self._id = re.findall(
                    r"\d+", re.findall(r"attempt=\d+", str(self.content))[0]
                )[0]
            except IndexError:
                self._id = None
        return self._id
    ...
    def all_questions(self) -> set:
        if not self._all_questions:
            try:
                self._all_questions = set(
                    [
                        x[:-1]
                        for x in re.findall(r'\d+_sub\d+_answer"', str(self.content))
                    ]
                )
            except:
