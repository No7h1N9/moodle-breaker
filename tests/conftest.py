...
ALL_TASKS_PAGE_DIR = FIXTURE_PATH / "all_tasks_page"
...
@fixture
@pytest.mark.parametrize(
    "directory", INPUT_ONLY_DIR.iterdir(), scope="session", ids=lambda param: param.name
)
...
@fixture
@pytest.mark.parametrize(
    "directory", RADIO_ONLY_DIR.iterdir(), scope="session", ids=lambda param: param.name
)
...
@pytest.mark.parametrize(
    "directory",
    ALL_TASKS_PAGE_DIR.iterdir(),
    scope="session",
    ids=lambda param: param.name,
)
def all_tasks_page_directory(directory: Path):
    return directory
